...
class PartialConv2d(torch.nn.Module):
    def __init__(
        self, conv: torch.nn.Conv2d, out_channel_begin: int, out_channel_end: int
    ):
        """
        :param conv: 원본 레이어
        ...
        self.conv = quantize_dynamic(self.conv, {torch.nn.Conv2d}, dtype=torch.qint8)
...
class SplitConv2d(torch.nn.Module):
    def __init__(self, conv: torch.nn.Conv2d, out_channels_per_part: list[int]):
        ...
        for num_channels in out_channels_per_part:
            ...
            self.partial_convs.append(
                PartialConv2d(conv, out_channel_begin, out_channel_end)
            )
...
class WorkerNode:
    ...
    def request_inference(
        self, x: torch.Tensor, original_layer_name: str, part_index: int
    ):
...
class PartialConv2dProxy(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        self.record_runtime(
            total_runtime=end - start, local_computation_runtime=runtime
        )
        return result
    ...
    def receive_inference_result(self):
        ...
        self.record_runtime(
            total_runtime=request_end_time - self.request_start_time,
            local_computation_runtime=runtime,
        )
...
class DistributedConv2d(torch.nn.Module):
    ...
    def __init__(
        ...
        for part_index, worker_node in enumerate(worker_nodes):
            worker_runtime = self.partial_convs_runtime.create_subcategory(
                "worker {}".format(part_index)
            )
            self.partial_convs.append(
                PartialConv2dProxy(
                    original_layer_name, part_index, worker_node, worker_runtime
                )
            )
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.is_sequential:
            ...
            partial_outputs = [
                conv.receive_inference_result() for conv in self.partial_convs
            ]
...
class SplitResnet(torch.nn.Module):
    ...
    def create_split_conv(self, layer: torch.nn.Conv2d):
        ...
        out_channels_per_part = self.distribute_channels(
            total_channels, self.num_splits
        )
...
class DistributedResnet(torch.nn.Module):
    def __init__(
        ...
        for name, layer in self.split_model.split_conv_dict.items():
            rsetattr(
                ...
                name,
                self.create_distributed_conv(name, layer, is_sequential),
            )

    def create_distributed_conv(
        self, name: str, layer: SplitConv2d, is_sequential: bool
    ):
        num_splits = len(layer.out_channels_per_part)
        if num_splits > len(self.worker_nodes):
            raise ValueError(
                f"Not enough worker nodes for layer {name}. "
                f"Required: {num_splits}, Available: {len(self.worker_nodes)}"
            )

        return DistributedConv2d(
            name,
            self.worker_nodes[:num_splits],
            is_sequential,
    ...
    def analyze_overheads(
        self, input_shape: torch.Size, num_tests: int, outer_tqdm_progress: tqdm | None
    ):
        total_runtime = []
        local_computation = []
        network_overhead = []
        ...
        with torch.no_grad():
            ...
            for _ in tqdm(
                ...
                local_computation.append(
                    self.runtime_record.net_runtime_per_category("local computation")
                )
                network_overhead.append(
                    self.runtime_record.net_runtime_per_category("network overhead")
                )
                concat_overhead.append(
                    self.runtime_record.net_runtime_per_category("concat")
                )
        ...
        print(
            "- total_runtime: avg = {:.7f}, samples =".format(
                sum(total_runtime) / num_tests
            ),
            total_runtime,
        )
        print(
            "- local computation: avg = {:.7f}, samples =".format(
                sum(local_computation) / num_tests
            ),
            local_computation,
        )
        print(
            "- network overhead: avg = {:.7f}, samples =".format(
                sum(network_overhead) / num_tests
            ),
            network_overhead,
        )
        print(
            "- concat overhead: avg = {:.7f}, samples =".format(
                sum(concat_overhead) / num_tests
            ),
            concat_overhead,
        )
...
def assert_model_equality(
    ...
    model2.eval()
    with torch.no_grad():
        desc = "model equality assertion ({} vs {})".format(
            model1._get_name(), model2._get_name()
        )
...
def assert_split_conv_correctness():
    ...
    orig = quantize_dynamic(orig, {torch.nn.Conv2d}, dtype=torch.qint8)
    ...
    try:
        with tcp.create_server("localhost", port) as server:
            ...
            while True:
                ...
                with torch.no_grad():
                    start = time.time()
                    y = split.split_conv_dict[original_layer_name].partial_convs[
                        part_index
                    ](x)
                    end = time.time()
...
def run_test_on_distributed_env(
    num_workers: int,
    test: Callable[[list[WorkerNode], models.ResNet, SplitResnet], None],
    ...
    progress = tqdm(
        total=2, desc="initializing worker node model", file=sys.stdout, leave=False
    )
    ...
    progress = tqdm(
        total=num_workers,
        desc="initializing worker nodes",
        file=sys.stdout,
        leave=False,
    )
    workers = []
    ...
    try:
        worker_nodes = [
            WorkerNode("localhost", port_offset + i)
            for i in tqdm(
                range(num_workers),
                desc="connecting to worker nodes",
                file=sys.stdout,
                leave=False,
            )
        ]
        ...
        progress = tqdm(
            total=num_workers,
            desc="waiting worker node termination",
            file=sys.stdout,
            leave=False,
        )
...
def assert_distributed_resnet_correctness(
    worker_nodes: list[WorkerNode], orig: models.ResNet, split: SplitResnet
):
...
def measure_distributed_resnet_overheads(
    worker_nodes: list[WorkerNode], orig: models.ResNet, split: SplitResnet
):
    progress = tqdm(
        total=2, desc="initializing distributed models", file=sys.stdout, leave=False
    )
    ...
    progress = tqdm(
        total=2,
        desc="measuring distributed model overhead",
        file=sys.stdout,
        position=0,
        leave=False,
    )
    progress.set_postfix_str("running sequential version")
    distributed_sequential.analyze_overheads(
        input_shape, num_tests=5, outer_tqdm_progress=progress
    )

    progress.set_postfix_str("running parallel version")
    distributed_parallel.analyze_overheads(
        input_shape, num_tests=5, outer_tqdm_progress=progress
    )
    progress.close()

    print(
        f"sequential version distributed part total runtime: {distributed_sequential.runtime_record.net_runtime_per_category('partial convs'):.7f}"
    )
    for i in range(len(worker_nodes)):
        print(
            f"worker node {i} total runtime: {distributed_sequential.runtime_record.net_runtime_per_category(f'worker {i}'):.7f}"
        )

    print(
        f"parallel version distributed part total runtime: {distributed_parallel.runtime_record.net_runtime_per_category('partial convs'):.7f}"
    )
    for i in range(len(worker_nodes)):
        print(
            f"worker node {i} total runtime: {distributed_parallel.runtime_record.net_runtime_per_category(f'worker {i}'):.7f}"
        )
...
def calculate_accuracy_and_latency(
    model: torch.nn.Module, image_files: list[str], class_mapping: dict
):
    correct = 0
    total = 0
    latencies = []
    ...
    with torch.no_grad():
        progress_bar = tqdm(
            ...
            desc="Calculating accuracy and latency",
            file=sys.stdout,
            position=1,
            leave=False,
        )
        ...
        for image_file in image_files:
            ...
            file_name = os.path.basename(
                image_file
            )  # 파일명 예: 'n01440764_tench.JPEG' 또는 'n01622779_great_grey_owl.JPEG'
            true_label = "_".join(file_name.split("_")[1:]).split(".")[
                0
            ]  # 'tench' 또는 'great_grey_owl'
...
def measure_distributed_resnet_accuracy_and_latency(
    worker_nodes: list[WorkerNode], orig: models.ResNet, split: SplitResnet
):
    progress = tqdm(
        total=3,
        desc="measuring distributed model accuracy and latency",
        file=sys.stdout,
        position=0,
    )
    ...
    image_files = [
        os.path.join(image_repo_path, f)
        for f in os.listdir(image_repo_path)
        if f.endswith(".JPEG")
    ]
    ...
    orig_accuracy, orig_latency = calculate_accuracy_and_latency(
        orig, image_files, class_mapping
    )
    progress.update()
    ...
    distributed_accuracy, distributed_latency = calculate_accuracy_and_latency(
        distributed, image_files, class_mapping
    )
    progress.update()
    progress.close()
...
if __name__ == "__main__":
    ...
    run_test_on_distributed_env(
        num_workers=3, test=measure_distributed_resnet_overheads
    )
